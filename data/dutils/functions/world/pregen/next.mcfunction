...
#save
execute store result storage dutils:temp pregen.step_x int 1 run scoreboard players get $world.temp_0 du_data
execute store result storage dutils:temp pregen.step_z int 1 run scoreboard players get $world.temp_1 du_data
...
execute if score $world.temp_1 du_data > $world.in_1 du_data run data remove storage dutils:temp pregen

#teleport player
summon area_effect_cloud -30000000 0 1600 {Duration:1,Tags:["du_teleport"]}
execute if score $world.temp_1 du_data <= $world.in_1 du_data as @e[tag=du_teleport] run function dutils:world/pregen/teleport
